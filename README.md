...
사용자별로 독립된 Podman 컨테이너를 생성하고, SSH로 접속할 수 있는 샌드박스 환경을 제공한다.
Cloudflare DNS를 통해 `{username}.sandbox.qucord.com` 형태의 서브도메인이 자동 등록된다.
...
```bash
# 서비스 파일 생성
cat > ~/.config/systemd/user/sandboxie.service << 'EOF'
[Unit]
...
[Service]
Type=simple
WorkingDirectory=/home/<user>/work/sandboxie
ExecStart=/home/<user>/.bun/bin/bun run backend/src/index.ts
EnvironmentFile=/home/<user>/work/sandboxie/.env
Restart=always
RestartSec=5
...
모든 API는 `Authorization: Bearer {token}` 헤더가 필요하다 (로그인 제외).
...
**접속 방법:**
```bash
ssh alice@alice.sandbox.qucord.com -p <assigned_port>
```
...
- **Cloudflare DNS**: `CF_API_TOKEN`과 `CF_ZONE_ID`가 설정되지 않으면 DNS 등록을 건너뛴다. DNS 실패는 세션 생성을 차단하지 않는다.
